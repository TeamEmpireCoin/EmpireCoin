...
{
    QString ret = QString("EmpireCoin:%1").arg(address);
    int paramCount = 0;
    ...
    if (ui->chkReqPayment->isChecked())
    {
        ...
        {
            // even if we allow a non EMP unit input in lnReqAmount, we generate the URI with EMP as unit (as defined in BIP21)
            ret += QString("?amount=%1").arg(EmpireCoinUnits::format(EmpireCoinUnits::EMP, ui->lnReqAmount->value()));
            paramCount++;
        }
        else
        {
