 ...
 * The EmpireCoin Developers 2011-2013
 */
...
bool WalletStack::addWallet(const QString& name, WalletModel *walletModel)
{
    ...
    walletView->setEmpireCoinGUI(gui);
...
void WalletStack::gotoVotingPage()
{
    QMap<QString, WalletView*>::const_iterator i;
    for (i = mapWalletViews.constBegin(); i != mapWalletViews.constEnd(); ++i)
...
void WalletStack::gotoEasyvotePage()
{
    QMap<QString, WalletView*>::const_iterator i;
    for (i = mapWalletViews.constBegin(); i != mapWalletViews.constEnd(); ++i)
        i.value()->gotoEasyvotePage();
}
