...
{
    // Update address book model from EmpireCoin core
...
QString AddressTableModel::addRow(const QString &type, const QString &label, const QString &address)
{
    ...
    if(type == Send)
    {
        ...
        {
            LOCK(wallet->cs_wallet);
            if(wallet->mapAddressBook.count(CEmpireCoinAddress(strAddress).Get()))
            {
                editStatus = DUPLICATE_ADDRESS;
                return QString();
            }
    ...
    {
        LOCK(wallet->cs_wallet);
        wallet->SetAddressBookName(CEmpireCoinAddress(strAddress).Get(), strLabel);
    }
