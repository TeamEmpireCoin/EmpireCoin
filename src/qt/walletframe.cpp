...
WalletFrame::WalletFrame(EmpireCoinGUI *_gui) :
    QFrame(_gui),
    gui(_gui),
    clientModel(0)
{
    ...
    walletStack->setEmpireCoinGUI(gui);
...
void WalletFrame::gotoVotingPage()
{
    WalletView *walletView = currentWalletView();
    if (walletView)
        walletStack->gotoVotingPage();
}

void WalletFrame::gotoEasyvotePage()
{
    walletStack->gotoEasyvotePage();
}
