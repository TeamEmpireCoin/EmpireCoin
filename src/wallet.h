...
public:
    ...
    std::map<CTxDestination, std::pair<std::string, std::string> > mapVotingAddressBook;
    ...
    bool SetVotingAddressBookName(
        const CTxDestination& address, const std::string& strName, const std::string& nation);
    ...
    /** Voting Address book entry changed.
     * @note called with lock cs_wallet held.
     */
    boost::signals2::signal<void (CWallet *wallet, const CTxDestination &address, const std::string &label, const std::string &nation, ChangeType status)> NotifyVotingAddressBookChanged;
