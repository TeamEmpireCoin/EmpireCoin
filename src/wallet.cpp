...
void CWalletTx::RelayWalletTransaction()
{
    ...
    {
        // Important: versions of EmpireCoin before 0.8.6 had a bug that inserted
        ...
        LOCK2(cs_main, cs_wallet);
        {
            ...
            loop
            {
                ...
                if (nChange > 0)
                {
                    ...
                    // change transaction isn't always pay-to-EmpireCoin-address
                    ...
                    else
                    {
                        ...
                        // Empirecoin also adds a restriction that
                        // change addresses cannot be voting addresses.
                        while(true)
                        {
                            assert(reservekey.GetReservedKey(vchPubKey)); // should never fail, as we just unlocked

                            scriptPubKey = CScript() << vchPubKey << OP_CHECKSIG;
...
{
    std::pair<std::string, std::string> labels = make_pair(strName, nation);
    std::map<CTxDestination, std::pair<std::string, std::string> >::iterator mi = mapVotingAddressBook.find(address);
    // if passed a blank nation, keep the original entry (likely called from VotingTableModel::setData)
    if (nation.empty() && (mi != mapVotingAddressBook.end())) {
        labels.second = mi->second.second;
    }
    mapVotingAddressBook[address] = labels;
    NotifyVotingAddressBookChanged(
        this, address, labels.first, labels.second, (mi == mapVotingAddressBook.end() ? CT_NEW : CT_UPDATED));
    if (!fFileBacked)
        return false;
    return CWalletDB(strWalletFile).WriteVotingName(CEmpireCoinAddress(address).ToString(), labels);
}
...
bool CWallet::DelAddressBookName(const CTxDestination& address)
{
    ...
    if (!fFileBacked)
        return false;
    return CWalletDB(strWalletFile).EraseName(CEmpireCoinAddress(address).ToString());
}
