...
bool CWalletDB::WriteVotingName(
    const string& strAddress, const std::pair<std::string, std::string>& labels)
{
    nWalletDBUpdated++;
    std::string votingData = labels.first + ":" + labels.second;
    return Write(make_pair(string("votingname"), strAddress), votingData);
}
             ...
             bool& fIsEncrypted,  bool& fAnyUnordered, string& strType, string& strErr)
{
    try {
        ...
        if (strType == "name")
        {
            string strAddress;
            ssKey >> strAddress;
            ssValue >> pwallet->mapAddressBook[CEmpireCoinAddress(strAddress).Get()];
        }
        else if (strType == "votingname")
        {
            string strAddress;
            string strValue;
            ssKey >> strAddress;
            ...
            boost::algorithm::split(pieces, strValue, boost::algorithm::is_any_of(":"));
            if (pieces.size() != 2) {
                strErr = "Error reading voting address data: Invalid entry found";
                return false;
            }
            std::pair<std::string, std::string> labels;
            labels.first = pieces[0];
            labels.second = pieces[1];
            pwallet->mapVotingAddressBook[CEmpireCoinAddress(strAddress).Get()] = labels;
        }
        else if (strType == "tx")
        {
            uint256 hash;
...
void ThreadFlushWalletDB(const string& strFile)
{
    ...
    RenameThread("EmpireCoin-wallet");
